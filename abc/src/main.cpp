...
class MainOutputTableABC : public MainOutputTable
{
	public:
		...
		MainOutputTableABC(int dim): MainOutputTable(dim)
		{
			std::vector<std::string> order =
				{"d", "b", "m", "p", "r", "ncells", "ndofs", "L2", "H1"};
		...
		for(unsigned int m = 1; m < 6;  m++)
		{
			...
			for (unsigned int p = 1; p < 4; p++)
			{
				...
				for (unsigned int r = 1; r < 4; r++)
				{
					...
					SolverABC<DIMENSION__> problem(m, p, 2, r,
				...
				table_PHI.save(dir + "table_PHI_m" + std::to_string(m) +
					"_p" + std::to_string(p));
			}
		}
	}
