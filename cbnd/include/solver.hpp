 ...
 *****************************************************************************/
template<int dim>
class SolverCBND : public SettingsCBND, public Solver<dim>
{
public:
...
template< int dim>
void SolverCBND<dim>::fill_dirichlet_stack()
{
#if IS_BC_EXACT__ == 1
	Solver<dim>::dirichlet_stack =
			{{bid_in, & exact_solution},
...
template <int dim>
void SolverCBND<dim>::data_slice(std::string fname)
{
...
template< int dim>
void SolverCBND<dim>::attach_manifold_refine()
{
