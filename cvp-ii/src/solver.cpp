...
void SolverCVPII::make_mesh()
{
	...
	{
		cell->set_material_id(mid_1); // The cell is outside the current-carrying region.

		if ((cell->center().norm() > a1) &&
				(cell->center().norm() < a2))
			cell->set_material_id(mid_2); // The cell is inside the current-carrying region.
...
void SolverCVPII::fill_dirichlet_stack()
{
	Solver<2>::dirichlet_stack =
		{{bid_dirichlet, & dirichlet_bc}};
}

void SolverCVPII::solve()
{
