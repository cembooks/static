...
template<>
double ExactSolutionINTAXI_PHI<true>::value(const Point<2> & p,
...
template<>
Tensor<1, 2> ExactSolutionINTAXI_PHI<true>::gradient(const Point<2> & p,
	const unsigned int component) const
{
	Point<2> r;
	r[0] = p[0];
...
template<>
double ExactSolutionINTAXI_PHI<false>::value(const Point<2> & p,
	const unsigned int component) const
{
	Point<2> r;
	r[0] = p[0];
...
template<>
Tensor<1, 2> ExactSolutionINTAXI_PHI<false>::gradient(const Point<2> & p,
	const unsigned int component) const
{
	Point<2> r;
	r[0] = p[0];
...
template<>
void ExactSolutionINTAXI_E<true>::vector_value_list(const std::vector<Point<2>> & r,
...
template<>
void ExactSolutionINTAXI_E<false>::vector_value_list(const std::vector<Point<2>> & r,
	std::vector<Vector<double>> &values) const
{
	Assert(values.size() == r.size(), ExcDimensionMismatch(values.size(), r.size()));
...
template<>
void ExactSolutionINTAXI_D<true>::vector_value_list(const std::vector<Point<2>> & r,
	std::vector<Vector<double>> &values) const
{
	Assert(values.size() == r.size(), ExcDimensionMismatch(values.size(), r.size()));
...
template<>
void ExactSolutionINTAXI_D<false>::vector_value_list(const std::vector<Point<2>> & r,
	std::vector<Vector<double>> &values) const
{
	Assert(values.size() == r.size(), ExcDimensionMismatch(values.size(), r.size()));
