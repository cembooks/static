...
template<>
double ExactSolutionINT_PHI<2>::value(const Point<2> & r,
...
template<>
Tensor<1, 2> ExactSolutionINT_PHI<2>::gradient(const Point<2> & r,
	const unsigned int component) const
{
	if ( r.norm() < d )
...
template<>
double ExactSolutionINT_PHI<3>::value(const Point<3> & r,
	const unsigned int component) const
{
	if ( r.norm() < d )
...
template<>
Tensor<1, 3> ExactSolutionINT_PHI<3>::gradient(const Point<3> & r,
	const unsigned int component) const
{
	if ( r.norm() < d )
...
template<>
void ExactSolutionINT_E<2>::vector_value_list(const std::vector<Point<2>> & r,
...
template<>
void ExactSolutionINT_E<3>::vector_value_list(const std::vector<Point<3>> & r,
	std::vector<Vector<double>> &values) const
{
	Assert(values.size() == r.size(), ExcDimensionMismatch(values.size(), r.size()));
...
template<>
void ExactSolutionINT_D<2>::vector_value_list(const std::vector<Point<2>> & r,
	std::vector<Vector<double>> &values) const
{
	Assert(values.size() == r.size(), ExcDimensionMismatch(values.size(), r.size()));
...
template<>
void ExactSolutionINT_D<3>::vector_value_list(const std::vector<Point<3>> & r,
	std::vector<Vector<double>> &values) const
{
	Assert(values.size() == r.size(), ExcDimensionMismatch(values.size(), r.size()));
