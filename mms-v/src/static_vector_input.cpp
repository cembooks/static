	...
	std::vector<Tensor<1,3>> & values) const
{
	...
	for (auto p: r)
	{
		(*v)[0] = 0.0;
		(*v)[1] = 0.0;
		(*v)[2] = (cos(k*p[0])+cos(k*p[1]))/((mu_0*(pow(p[0],2)+pow(p[1],2)+1.0)));
		v++;
	}
}
	...
	std::vector<Tensor<1,2>> & values) const
{
	...
	for (auto p: r)
	{
		(*v)[0] = (cos(k*p[0])+cos(k*p[1]))/((mu_0*(pow(p[0],2)+pow(p[1],2)+1.0)));
		(*v)[1] = 0.0;
		v++;
	}
}
	...
	std::vector<Tensor<1,2>> & values) const
{
	...
	for (auto p: r)
	{
		...
		Q[0] =	(*nn)[1] * T + gamma * (*nn)[1] * ( (*nn)[0] * A[1] - (*nn)[1] * A[0] );
		Q[1] = -(*nn)[0] * T - gamma * (*nn)[0] * ( (*nn)[0] * A[1] - (*nn)[1] * A[0] );
