...
void SolverMMSVTII_T::make_mesh()
{
	GridIn<2> gridin;
...
#if DOMAIN__ == 0
	std::ofstream ofs("../../gmsh/data/circle_r" + std::to_string(r) + "_reordered.msh");
#elif DOMAIN__ == 1
	std::ofstream ofs("../../gmsh/data/square_r" + std::to_string(r) + "_reordered.msh");
#endif
	gridout.write_msh(Solver<2,0>::triangulation, ofs);
}

void SolverMMSVTII_T::fill_dirichlet_stack()
{
	Solver<2,0>::dirichlet_stack =
		{{bid_dirichlet, & dirichlet_bc}};
}

void SolverMMSVTII_T::solve()
{
...
void SolverMMSVTII_A::fill_dirichlet_stack()
{
	Solver2<2,2>::dirichlet_stack =
		{{bid_dirichlet, & dirichlet_bc}};
}

void SolverMMSVTII_A::solve()
{
	ReductionControl control(
