...
template<>
void StaticVectorSolver::TheCoefficient<2, 2>::value_list(
...
template<>
void StaticVectorSolver::PdeRhs<2, 2>::value_list(
	const std::vector<Point<2>> &r,
	types::material_id mid,
	unsigned int cuid,
...
template<>
void StaticVectorSolver::Gamma<2, 2>::value_list(
	const std::vector<Point<2>> &r,
...
template<>
void StaticVectorSolver::RobinRhs<2, 2>::value_list(
	const std::vector<Point<2>> &r,
	const std::vector<Tensor<1, 2>> & n,
	types::boundary_id bid,
	types::material_id mid,
	...
	std::vector<Tensor<1,2>> & values) const
{
	...
	for (unsigned int i = 0; i < r.size(); i++)
	{
		...
		values.at(i)[0] =	 n.at(i)[1]*T + gamma*n.at(i)[1]*(n.at(i)[0]*A[1]-n.at(i)[1]*A[0]);
		values.at(i)[1] = -n.at(i)[0]*T - gamma*n.at(i)[0]*(n.at(i)[0]*A[1]-n.at(i)[1]*A[0]);
	}
}

template<>
void StaticVectorSolver::FreeSurfaceCurrent<2, 2>::value_list(
	const std::vector<Point<2>> &r,
	const std::vector<Tensor<1,2>> & n,
	types::material_id mid,
	unsigned int cuid,
...
template<>
double StaticVectorSolver::Weight<2, 2>::value(const Point<2> & r,
...
template<>
double StaticVectorSolver::Weight<2, 3>::value(const Point<2> & r,
	const unsigned int component) const
{
	return 1.0;
}
