...
{
public:
	QuadratureTableScalar(unsigned int p): indx(p-1)
	{
		Assert( indx < 4, dealii::ExcInternalError() );
	}

	QuadratureTableScalar(const QuadratureTableScalar<dim> & qt): indx(qt.indx)
	{
		Assert( indx < 4, dealii::ExcInternalError() );
	}
...
{
public:
	QuadratureTableVector(unsigned int p): indx(p)
	{
		Assert( indx < 3, dealii::ExcInternalError() );
	}

	QuadratureTableVector(const QuadratureTableVector<dim> & qt): indx(qt.indx)
	{
		Assert( indx < 3, dealii::ExcInternalError() );
	}
