 ...
 * that \f$\vec{B}\f$ is expressed as a linear combination of the shape functions
 * of the
 ...
 * \f$\vec{A}\f$ from the \f$H(\text{curl})\f$ function space, calculates its curl,
 * and projects the result into \f$H(\text{div})\f$ function space, which is a
 * proper space for the magnetic field \f$\vec{B}\f$. The operation of projection
 * is illustrated by  means of the Bossavit's diagram below.
 *
 ...
 * the degrees of freedom are accessible while \f$\vec{B}\f$ is computed. An object of
 * the StaticVectorSolver::ProjectAtoB class template reuses the triangulation by
 * creating an additional dof handler associated with the
 * [FE_RaviartThomas](https://www.dealii.org/current/doxygen/deal.II/classFE__RaviartThomas.html)
 * finite elements. That is, \f$\vec{A}\f$ and \f$\vec{B}\f$ share the same triangulation
