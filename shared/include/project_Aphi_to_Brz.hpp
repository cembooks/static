...
#ifndef ProjectAphiToBrz_H__
#define ProjectAphiToBrz_H__

#include "project_Hgrad_to_Hdiv.hpp"

namespace StaticScalarSolver {
/**
 * \brief Computes the two-dimensional scaled magnetic field \f$\vec{B}'\f$
 * as a vector curl of the scaled magnetic vector potential, \f$A'\f$, i.e.,
 * \f$\vec{B}' = -\vec{\nabla} \overset{V}{\times} A'\f$.
 *
 * The problem is assumed to be axisymmetric two-dimensional. The scaled
 * magnetic field \f$\vec{B}'\f$ is an in-plane vector. The scaled magnetic
 ...
 * function space. This class template computes scaled magnetic field
 * \f$\vec{B}'\f$ such that \f$\vec{B}'\f$ is expressed as a linear combination
 ...
 * the curl by -1, and projects the result into \f$H(\text{div})\f$ function
 * space, which is a proper space for the scaled magnetic field \f$\vec{B}'\f$.
 * The operation of projection is illustrated by means of the Bossavit's
 * diagram below.
 *
 * ![](sss_prj/diagram_prj_Aphi_to_Brz.svg)
 *
 ...
 * triangulation but are modeled by different finite elements: \f$\vec{B}'\f$ -
 * by
 * [FE_RaviartThomas](https://www.dealii.org/current/doxygen/deal.II/classFE__RaviartThomas.html)
 * and \f$A'\f$ - by
 * [FE_Q](https://www.dealii.org/current/doxygen/deal.II/classFE__Q.html).
 ...
 * `project_exact_solution` passed to the constructor equals true.
 *
...
class ProjectAphiToBrz : public ProjectHgradToHdiv<2, stage>
{
public:
  ProjectAphiToBrz() = delete;
  /**
   ...
   * result into the output file next to \f$\vec{B}'\f$.
