 ...
 * the magnetic field has only \f$z\f$ component and the magnetic vector potential
 * has only \f$x\f$ and \f$y\f$ components.
 *
 * This class template
 ...
 * that \f$B\f$ is expressed as a linear combination of the shape functions of the
 ...
 * scalar curl, and projects the result into \f$L^2\f$ function space, which is a
 * proper space for the out-of-plane magnetic field \f$B\f$. The operation of
 ...
 * StaticVectorSolver::Solver2 that yielded \f$\vec{A}\f$ is still in the computer
 * memory such that the triangulation, the degrees of freedom, and the handler of
 * the degrees of freedom are accessible while \f$B\f$ is computed. An object of
 * the StaticVectorSolver::ProjectAxyToBz class template reuses the triangulation by
 * creating an additional dof handler associated with the
 * [FE_DGQ](https://www.dealii.org/current/doxygen/deal.II/classFE__DGQ.html)
 * finite elements. That is, \f$\vec{A}\f$ and \f$B\f$ share the same triangulation
 * but are modeled by different finite elements: \f$\vec{A}\f$ - by
 * [FE_Nedelec](https://www.dealii.org/current/doxygen/deal.II/classFE__Nedelec.html)
 * and
 * \f$B\f$ - by
 * [FE_DGQ](https://www.dealii.org/current/doxygen/deal.II/classFE__DGQ.html)
 ...
 *   i.e., FE_DGQ.
 *
 * The "L2norm", and "ScalarFieldExact" are saved only if an exact solution is
 * passes to the constructor.  Moreover, "ScalarFieldExact" is calculated and
 ...
 * object of the class StaticVectorSolver::Solver1 or StaticVectorSolver::Solver2
 * that has yielded the magnetic vector potential, \f$\vec{A}\f$.
