 ...
 * two-dimensional problems formulated in terms of the magnetic vector
 *potential, \f$\vec{A}\f$, the numerically calculated potential needs to be
 *converted into magnetic field, \f$B\f$, as \f[ B = \vec{\nabla}
 *\overset{S}{\times} \vec{A}. \f] The magnetic vector potential belongs to the
 *H(curl) function space. The magnetic field belongs to the \f$L^2\f$ function
 *space. Therefore, one needs to compute the equation above such that the input,
 *\f$\vec{A}\f$, is in H(curl) and the output, \f$B\f$, is in \f$L^2\f$. Such
 *computation can be envisioned as a some kind of projection from one function
 *space, H(curl), into another, \f$L^2\f$.
 *
