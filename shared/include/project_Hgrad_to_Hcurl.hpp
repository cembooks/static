...
template<int dim, int stage>
void ProjectHgradToHcurl<dim, stage>::setup()
{
	constraints.close();
	...
	DynamicSparsityPattern dsp(dof_handler_Hcurl.n_dofs(), dof_handler_Hcurl.n_dofs());
...
template<int dim, int stage>
void ProjectHgradToHcurl<dim, stage>::solve()
{
...
template<int dim, int stage>
void ProjectHgradToHcurl<dim, stage>::save() const
{
...
template<int dim, int stage>
void ProjectHgradToHcurl<dim, stage>::compute_error_norms()
{
	Weight<dim,stage> weight;
	const Function<dim, double> * mask = & weight;

	Constants::QuadratureTableVector<dim> qt(dof_handler_Hcurl.get_fe().degree - 1);
...
template<int dim, int stage>
void ProjectHgradToHcurl<dim, stage>::project_exact_solution_fcn()
{
...
ProjectHgradToHcurl<dim, stage>::AssemblyScratchData::AssemblyScratchData(
	...
	unsigned int mapping_degree):
		...
		qt(fe.degree - 1),
		fe_values_Hcurl(mapping, fe, QGauss<dim>(qt.sim()),
			update_values |
...
template<int dim, int stage>
void ProjectHgradToHcurl<dim, stage>::system_matrix_local(
	const IteratorPair & IP,
...
// (3) Recipe for projections from H(grad) to H(curl) nr. 3 and nr. 4 (axisym.)
// (4) Recipe for projections from H(grad) to H(curl) nr. 5 (planar)
...
// recipe (2), etc.
	...
	for (unsigned int q_index = 0; q_index < scratch_data.n_q_points; ++q_index)
	{
		...
		if ((scratch_data.axisymmetric)&&(!scratch_data.vector_potential))
			scratch_data.axi_mult = scratch_data.fe_values_Hcurl.quadrature_point(q_index)[0];
		...
		for (unsigned int i = 0; i < scratch_data.dofs_per_cell; ++i)
		{
			...
			if (scratch_data.vector_potential) // A and A'.
			{
				...
				double tmp;
				tmp =
					scratch_data.the_coefficient_list[q_index] *                // 1 / mu
...
//	Note, that a quadrature point can, in general, be at the origin. If so, the line below will
//	yield an error.
				copy_data.cell_rhs(i) -= tmp * scratch_data.fe_values_Hcurl.quadrature_point(q_index)[0];

// The recipe (5) yields H'=rH=(1\mu)rB , where r is the distance to the axis of rotation symmetry.
// If you do not like to have the scaled H-field, H', and would rather have the H-field itself, H,
...
template<int dim, int stage>
void ProjectHgradToHcurl<dim, stage>::copy_local_to_global(const AssemblyCopyData &copy_data)
{
...
template<int dim, int stage>
void ProjectHgradToHcurl<dim, stage>::assemble()
{
	WorkStream::run(
		IteratorPair(IteratorTuple(dof_handler_Hcurl.begin_active(),
...
template<int dim, int stage>
void ProjectHgradToHcurl<dim, stage>::save_matrix_and_rhs_to_csv(std::string fname) const
{
