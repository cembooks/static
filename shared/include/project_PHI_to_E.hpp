 ...
 * that \f$\vec{E}\f$ is expressed as a linear combination of the shape
 * functions of the
 ...
 * the StaticScalarSolver::ProjectPHItoE class template reuses the triangulation
 * by creating an additional dof handler associated with the
 * [FE_Nedelec](https://www.dealii.org/current/doxygen/deal.II/classFE__Nedelec.html)
 * finite elements. That is, \f$\vec{E}\f$ and \f$\Phi\f$ share the same
 * triangulation but are modeled by different
 * finite elements: \f$\vec{E}\f$ - by
 * [FE_Nedelec](https://www.dealii.org/current/doxygen/deal.II/classFE__Nedelec.html)
 * and \f$\Phi\f$ - by
 * [FE_Q](https://www.dealii.org/current/doxygen/deal.II/classFE__Q.html).
...
public:
   ...
   * result into the output file next to \f$\vec{E}\f$.
