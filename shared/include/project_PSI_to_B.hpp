 ...
 * the StaticScalarSolver::ProjectPSItoB class template reuses the triangulation by
 * creating an additional dof handler associated with the
 * [FE_RaviartThomas](https://www.dealii.org/current/doxygen/deal.II/classFE__RaviartThomas.html)
 * finite elements. That is, \f$\vec{B}\f$ and \f$\Psi\f$ share the same triangulation
 ...
 * evaluated at the quadrature points and interpreted as permeability, \f$\mu\f$,
 * which is necessary ingredient for computing \f$\vec{B}\f$, see above.
 *
