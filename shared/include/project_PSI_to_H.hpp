 ...
 * \f$H(\text{curl})\f$ function space, which is a proper space for the auxiliary
 * vector field \f$\vec{H}\f$. The operation of projection is illustrated by
 ...
 * the StaticScalarSolver::ProjectPSItoH class template reuses the triangulation by
 * creating an additional dof handler associated with the
 * [FE_Nedelec](https://www.dealii.org/current/doxygen/deal.II/classFE__Nedelec.html)
 * finite elements. That is, \f$\vec{H}\f$ and \f$\Psi\f$ share the same triangulation
