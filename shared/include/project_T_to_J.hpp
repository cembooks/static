 ...
 * \f$\vec{J}_f\f$ as a curl of the current vector potential, \f$\vec{T}\f$, such
 * that \f$\vec{J}_f\f$ is expressed as a linear combination of the shape functions
 * of the
 ...
 * \f$\vec{T}\f$ from the \f$H(\text{curl})\f$ function space, calculates its curl,
 * and projects the result into \f$H(\text{div})\f$ function space, which is a
 * proper space for the volume free-current density \f$\vec{J}_f\f$. The operation
 * of projection is illustrated by  means of the Bossavit's diagram below.
 *
 ...
 * the degrees of freedom are accessible while \f$\vec{J}_f\f$ is computed. An object of
 * the StaticVectorSolver::ProjectTtoJ class template reuses the triangulation by
 * creating an additional dof handler associated with the
 * [FE_RaviartThomas](https://www.dealii.org/current/doxygen/deal.II/classFE__RaviartThomas.html)
 * finite elements. That is, \f$\vec{T}\f$ and \f$\vec{J}_f\f$ share the same triangulation
 * but are modeled by different finite elements: \f$\vec{T}\f$ - by
 * [FE_Nedelec](https://www.dealii.org/current/doxygen/deal.II/classFE__Nedelec.html)
 * and
 * \f$\vec{J}_f\f$ - by
 * [FE_RaviartThomas](https://www.dealii.org/current/doxygen/deal.II/classFE__RaviartThomas.html).
 ...
 *   \f$\vec{J}_f = \vec{\nabla} \times \vec{T}\f$, under the name "VectorField".
