 ...
 * is an out-of-plane vector. This class template is meant to be used in conjunction
 * with StaticScalarSolver::Solver.
 *
 ...
 * \f$\vec{J}_f\f$ such that \f$\vec{J}_f\f$ is expressed as a linear combination
 ...
 * the StaticScalarSolver::ProjectTzToJxy class template reuses the triangulation by
 * creating an additional dof handler associated with the
 * [FE_RaviartThomas](https://www.dealii.org/current/doxygen/deal.II/classFE__RaviartThomas.html)
 * finite elements. That is, \f$\vec{J}_f\f$ and \f$T\f$ share the same triangulation
