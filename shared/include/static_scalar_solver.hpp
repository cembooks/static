 ...
 *
 * This class template is intended to be a general solver for problems in electro-
 * and magnetostatics that can be formulated in therms of the electrostatic
 * scalar potential, \f$\Phi\f$, total magnetostatic scalar potential,\f$\Psi\f$,
 * reduced magnetostatic scalar potential, \f$\Theta\f$, two-dimensional
 * magnitude of vector potential, \f$A\f$, and scaled two-dimensional magnitude of
 * vector potential, \f$A'\f$. It can also be used to solve for the current
 * vector potential, \f$T\f$, in planar two-dimensional problems. The calculated
 * potential is saved in a vtk file, see function save() for more details.
 * The Bossavit's diagrams below illustrate the partial differential equations
 * that can be solved with a help of this class template. Note, that in all
 * cases listed below the potential belongs to the \f$H(\text{grad})\f$ function
 * space, i.e., is modeled by the
 ...
 *   even boundary ID's. The boundary ID's in this case must be greater than zero.
 *   The Neumann boundary conditions are considered to be special cases of the
 *   Robin boundary conditions with \f$\gamma=0\f$.
 ...
 *   setting \f$ \sigma \f$ and \f$ \gamma \f$ to zero in the value_list(...) methods
 *   of the class templates StaticScalarSolver::RobinRhs
 *   and StaticScalarSolver::Gamma.
 *
 ...
 * density \f$\rho_f\f$. The corresponding integral in the variational formulation
 * reads
 * \f[
 ...
 * \f]
 * in two dimensions. In the first mode the objects derived form this class
 * template evaluate the values of \f$\rho_f\f$ at quadrature points by calling
 ...
 * details see the discussion that led to the color box "Recipe for static scalar
 * solver in 2D (current vect. potential)" in the book):
 * \f[
 ...
 * \oint_{\Gamma} \vec{J}_f \cdot \bigg(\hat{n} \overset{V}{\times} T \bigg) dl
 * \f]
 * That is to say, in the second mode the input at the right-hand side of
 * the partial differential equation is not a scalar field (such as \f$\rho_f\f$),
 * but a two-dimensional vector field, \f$\vec{J}_f\f$. The class template calls
 * an object of the type StaticScalarSolver::PdeRhsCvp to evaluate the values
 * of \f$\vec{J}_f\f$ at quadrature points. Setting type_pde_rhs =0 is the same
 * as type_pde_rhs =1 and \f$\rho_f = 0\f$. In this case algorithm saves
 ...
 * - [sld-ii/](@ref page_sld_ii) - Total scalar magnetic potential, \f$\Theta\f$,
 *   in three-dimensional and planar two-dimensional problems.
 * - [mwr/](@ref page_mwr) - Magnetic vector potential, \f$A\f$, in planar two-dimensional
 *   problems.
 * - [ssol-a/](@ref page_ssol_a) - Scaled magnetic vector potential, \f$A'\f$, in axisymmetric
 *   two-dimensional problems.
 ...
 *
 * @param[in] p - The degree of the interpolating Lagrange polynomials in finite
 * elements that model the potential.
 ...
 * @param[in] type_of_pde_rhs - Toggles between the two modes of operation, see
 * above.
 ...
 * result into the vtk file next to the solution. This may be useful for debugging
 * purposes as a comparison between the projected exact solution and the
 * solution to the boundary value problem can yield a hint on where to search
 * for bugs.
 *****************************************************************************/
	Solver(
			unsigned int p,
...
"The setting axisymmetric=true is only allowed if dim=2."));
				...
				Assert( type_of_pde_rhs < 2, ExcMessage(
...
/**
 * \brief Initializes the data member StaticScalarSolver::Solver::triangulation.
 *
 * This function must be overridden by the user. It must generate or load the
 * mesh, label the boundaries, and, if necessary, assign user IDs. This function
 * is an ideal place for binding manifolds to the mesh. The last is a reasonable
 * thing to do if degree_mapping>1. The mesh must be stored in the data
 * member of this class StaticScalarSolver::Solver::triangulation.
 *****************************************************************************/
	virtual void make_mesh() = 0;

/**
 * \brief Initializes the data member StaticScalarSolver::Solver::dirichlet_stack.
 *
 ...
 * exact solution is submitted as the third input parameter to the constructor.
 * Moreover, "ScalarFieldExact" is calculated and saved only if the last input
 * parameter, project_exact_solution, of the constructor equals true.
 *
...
// ----------------------------------------------------------------------------
	struct AssemblyScratchData
	{
		...
		double axi_mult; // Equals the distance to the axis of rotation symmetry, r,
		                 // in the "Recipe for static scalar solver in 2D (axisymmetric)".
		                 // Equals 1.0 in all other recipes. All integrands are
...
template<int dim, int stage>
void Solver<dim, stage>::assemble()
{
	WorkStream::run(
...
template<int dim, int stage>
void Solver<dim, stage>::system_matrix_local(
	const typename DoFHandler<dim>::active_cell_iterator &cell,
...
// recipe (2), recipe (3), and recipe (4).
	...
	{ // The coefficient equals 1.0 if the current vector potential, T, is computed.
	...
	for (unsigned int q_index = 0; q_index < scratch_data.n_q_points; ++q_index)
	{
		...
		for (unsigned int i = 0; i < scratch_data.dofs_per_cell; ++i)
		{
			for (unsigned int j = 0; j < scratch_data.dofs_per_cell; ++j)
			{
				...
				copy_data.cell_matrix(i, j) += scratch_data.axi_mult *
			...
			switch(scratch_data.type_of_pde_rhs)
			{
				...
				case 1:
					...
					copy_data.cell_rhs(i) += scratch_data.axi_mult *
						scratch_data.pde_rhs_list[q_index] *              // rho
...
". Only the following values are allowed: 0, 1, and 2."));
	...
	for (unsigned int f = 0; f < GeometryInfo<dim>::faces_per_cell; ++f)
	{
		...
		Assert(!(scratch_data.do_robin && scratch_data.do_kappa), ExcMessage(
...
Therefore, do_robin and do_kappa are mutually exclusive."));
		...
		Assert(!(scratch_data.do_kappa && scratch_data.do_Jf_on_boundary), ExcMessage(
...
do_Jf_on_boundary and do_kappa are mutually exclusive."));
		...
		if (scratch_data.do_robin || scratch_data.do_kappa || scratch_data.do_Jf_on_boundary)
		{
				...
				for (unsigned int i = 0; i < scratch_data.dofs_per_cell; ++i)
				{
					if (scratch_data.do_robin)
					{
						for (unsigned int j = 0; j < scratch_data.dofs_per_cell; ++j)
						{
							...
							copy_data.cell_matrix(i, j) += scratch_data.axi_mult *
								...
								scratch_data.fe_face_values.shape_value(j, q_index_face) * // N_j
					...
					if (scratch_data.do_Jf_on_boundary)
					{
						...
						copy_data.cell_rhs(i) -=
							...
							scratch_data.fe_face_values.normal_vector(q_index_face)[0] //        V
							)*scratch_data.fe_face_values.JxW(q_index_face);           // J_f.(n x N_i)dl =
							                                                           //           S
							                                                           // = N_i(J_f x n)dl
					}
...
template <int dim, int stage>
void Solver<dim, stage>::copy_local_to_global(const AssemblyCopyData &copy_data)
{
...
template <int dim, int stage>
void compute_Linfty_error_norm(
	const DoFHandler<dim> & dof_handler,
	const Triangulation<dim> & triangulation,
	const Vector<double> & solution,
	const Function<dim> * exact_solution,
...
template<int dim, int stage>
void Solver<dim, stage>::save() const
{
...
template<int dim, int stage>
void Solver<dim, stage>::save_matrix_and_rhs_to_csv(std::string fname) const
{
