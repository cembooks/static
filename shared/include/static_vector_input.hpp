...
public:
   ...
   * <code>values</code> with the values of the coefficient.
   * The <code>values[i]</code> is interpreted the coefficient at quadrature
   * point <code>r[i]</code>. The code snippet below provides an example in
   * which permeability equals
   *
 ...
 * the parameter <code>type_of_pde_rhs</code>. It can be interpreted as current
 * density, \f$\vec{J}_f\f$, or as the current vector potential, \f$\vec{T}\f$.
 * See the documentation of StaticVectorSolver::Solver1 for more details.
 * The StaticVectorSolver::Solver2 class template does not use
 * StaticVectorSolver::PdeRhs as the right-hand-side of the PDE is fed to
 * the StaticVectorSolver::Solver2 via the
 * <code>dof_handler</code> and <code>solution</code>.
 *
 *
 * This class is declared in shared/include/static_vector_input.hpp but
 * must be implemented in xyz/src/static_vector_input.cpp, where xyz is
  ...
  , private Settings
{
   ...
   * the assembly of the right-hand side. It is called ones per each cell.
   * This function must fill <code>values</code> vector with the source vectors
   * sampled at quadrature points. The <code>values[i]</code> is interpreted as
   * the source vector at quadrature point <code>r[i]</code>. The code snippet
   * below provides an example in which the source field equals
   *
   * \f[
...
public:
   ...
   * This function must fill the vector <code>values</code> with the values of
   * \f$\gamma\f$. The <code>values[i]</code> is interpreted as \f$\gamma\f$
   * at quadrature point <code>r[i]</code>. The code snippet below provides an
   * example in which \f$\gamma\f$ equals
   *
   * \f[
...
public:
   ...
   * [boundary ID convention](@ref veis_bnd_convention).
   * This function must fill the vector <code>values</code> with the values of
   * \f$\vec{Q}\f$. The <code>values[i]</code> is interpreted as \f$\vec{Q}\f$
   * at quadrature point <code>r[i]</code>. The code snippet below provides an
   * example in which \f$\vec{Q}\f$ equals zero.
   *
   * @code
   * #pragma GCC diagnostic push
   * #pragma GCC diagnostic ignored "-Wunused-parameter"
  ...
  , private Settings
{
   ...
   * dissimilar materials. This function must fill the vector
   * <code>values</code> with the values of \f$\vec{K}_f\f$. The
   * <code>values[i]</code> is interpreted as \f$\vec{K}_f\f$ at quadrature
   * point <code>r[i]</code>. The code snippet below provides an example in
   * which \f$\vec{K}_f\f$ equals zero.
   *
   * @code
   * #pragma GCC diagnostic push
   * #pragma GCC diagnostic ignored "-Wunused-parameter"
