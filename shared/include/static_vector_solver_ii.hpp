 ...
 *
 * This class template is intended to be a general solver for problems in magnetostatics
 * that can be formulated in terms of the magnetic vector potential, \f$\vec{A}\f$.
 * The Bossavit's diagrams below illustrate the partial differential equations that can
 * be solved with a help of this class template.
 *
 * ![](svst_svst/diagram_svst.svg)
 *
 * This class template is very similar to StaticVectorSolver::Solver1. The main
 * difference between StaticVectorSolver::Solver1 and StaticVectorSolver::Solver2
 * is that all inputs to StaticVectorSolver::Solver1 must be given in a form of
 * an analytical expression so they can be codded in class templates derived from
 ...
 * partial differential equation. The input on the right-hand side of the partial
 * differential equation fed into the StaticVectorSolver::Solver2 class template must
 * be a current vector potential, \f$\vec{T}\f$, expressed in a form of a field function.
 * That is, \f$\vec{T}\f$ fed into StaticVectorSolver::Solver2 is a result of another deal.II
 * simulation. This allows to solve the curl-curl equation for \f$\vec{A}\f$ observing the
 ...
 *   void value_list(...);
 *	 @endcode
 *   of the classes StaticVectorSolver::TheCoefficient, StaticVectorSolver::Gamma,
...
/**
 *	\brief Initializes the data member StaticVectorSolver::Solver2::dirichlet_stack.
 *
...
Solver2<dim, stage>::AssemblyScratchData::AssemblyScratchData(
		...
		qt(fe.degree-1),
		fe_values(mapping, fe, QGauss<dim>(qt.sim()),
			update_gradients |
			update_values |
			update_quadrature_points |
			update_JxW_values),
		fe_face_values(mapping, fe, QGauss<dim - 1>(qt.sim()),
			update_values |
			update_normal_vectors |
			update_quadrature_points |
			update_JxW_values),
		fe_values_T(mapping, dof_hand_T.get_fe(), QGauss<dim>(qt.sim()),
			update_values |
			update_gradients),
		fe_face_values_T(mapping, dof_hand_T.get_fe(), QGauss<dim-1>(qt.sim()),
			update_values |
			update_gradients),
...
Solver2<dim, stage>::AssemblyScratchData::AssemblyScratchData(
	  ...
	  qt(scratch_data.qt),
		fe_values(mapping, scratch_data.fe_values.get_fe(),
			scratch_data.fe_values.get_quadrature(),
			update_gradients |
			update_values |
			update_quadrature_points |
			update_JxW_values),
		fe_face_values(mapping, scratch_data.fe_face_values.get_fe(),
			scratch_data.fe_face_values.get_quadrature(),
			update_values |
			update_normal_vectors |
			update_quadrature_points |
			update_JxW_values),
		fe_values_T(mapping, scratch_data.fe_values_T.get_fe(),
			scratch_data.fe_values_T.get_quadrature(),
			update_values |
			update_gradients),
		fe_face_values_T(mapping, scratch_data.fe_face_values_T.get_fe(),
...
template<int dim, int stage>
void Solver2<dim, stage>::system_matrix_local(
	const IteratorPair & IP,
...
// and recipe (2).
	...
	if (dim == 2)
	{
		scratch_data.fe_values_T[SE].get_function_values(
			scratch_data.dofs_T,
			scratch_data.values);
	}else if (dim == 3)
	{
	...
	for (unsigned int q_index = 0; q_index < scratch_data.n_q_points; ++q_index)
	{
		for (unsigned int i = 0; i < scratch_data.dofs_per_cell; ++i)
		{
			...
			{// Integral I_a1+I_a3 in recipes (1) and (2).
			...
			if (scratch_data.do_T_on_boundary)
			{
				if (dim == 2)
				{
					scratch_data.fe_face_values_T.get_function_values(
						scratch_data.dofs_T,
						scratch_data.values_face);
				}else if (dim == 3)
				{
					scratch_data.fe_face_values_T[VE].get_function_values(
						scratch_data.dofs_T,
						scratch_data.vector_values_face);
				}
			}
					...
					if (scratch_data.do_robin)
					{
						for (unsigned int j = 0; j < scratch_data.dofs_per_cell; ++j)
						{
							...
							{// Integral I_a2 in recipe (2)
								...
								scratch_data.fe_face_values.JxW(q_index_face); // (n x N_i) . (n x N_j) dl
							}
							else if (dim == 3)
...
template<int dim, int stage>
void Solver2<dim, stage>::copy_local_to_global(const AssemblyCopyData &copy_data)
{
...
template<int dim, int stage>
void Solver2<dim, stage>::compute_error_norms()
{
...
template<int dim, int stage>
void Solver2<dim, stage>::project_exact_solution_fcn()
{
...
template<int dim, int stage>
void Solver2<dim, stage>::save() const
{
