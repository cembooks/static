...
template<>
ExactSolutionSLDII_THETA<2>::ExactSolutionSLDII_THETA() : Function<2>(), SettingsSLDII()
{
	double mur = mur_2;
...
template<>
double ExactSolutionSLDII_THETA<2>::value(const Point<2> &r,
	unsigned int component) const
{
	...
	if ( rn < a )
		return delta_1*r[0] + H_0*r[0];

	if ( rn > b )
		return (-H_0+alpha_1/r2)*r[0] + H_0*r[0];

	return (beta_1+gamma_1/r2)*r[0] + H_0*r[0];
}

template<>
Tensor<1,2> ExactSolutionSLDII_THETA<2>::gradient(const Point<2> &r,
	unsigned int component) const
{
	double rn = r.norm();
	double r2 = std::pow(rn,2.0);
	...
	if ( rn > b )
	{
		grad_phi[0] = -H_0+alpha_1/r2+alpha_1*xx + H_0 ;
		grad_phi[1] = alpha_1*xy;
		return grad_phi;
	}

	grad_phi[0] = beta_1+gamma_1/r2+gamma_1*xx + H_0;
	grad_phi[1] = gamma_1*xy;
	return grad_phi;
}

template<>
ExactSolutionSLDII_THETA<3>::ExactSolutionSLDII_THETA() : Function<3>(),
	SettingsSLDII()
{
	double mur = mur_2;
...
template<>
double ExactSolutionSLDII_THETA<3>::value(const Point<3> &r,
	unsigned int componet) const
{
	...
	if ( rn < a )
		return delta_1*r[2] + H_0*r[2];

	if ( rn > b )
		return (-H_0+alpha_1/r3)*r[2] + H_0*r[2];

	return (beta_1+gamma_1/r3)*r[2] + H_0*r[2];
}

template<>
Tensor<1,3> ExactSolutionSLDII_THETA<3>::gradient(const Point<3> &r,
	unsigned int component) const
{
	double rn = r.norm();
	double r3 = std::pow(rn,3.0);
	...
	if ( rn > b )
	{
		...
		grad_phi[2] = -H_0+alpha_1/r3+alpha_1*zz + H_0;
		return grad_phi;
	}

	grad_phi[0] = gamma_1*xz;
	grad_phi[1] = gamma_1*yz;
	grad_phi[2] = beta_1+gamma_1/r3+gamma_1*zz + H_0;
	return grad_phi;
}
