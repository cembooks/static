                                 ...
                                 std::vector<double>& values) const
{
  ...
  for (unsigned int i = 0; i < values.size(); i++) {
    if ((cuid == 1) && (fuid == 1)) {
      values[i] = -(mu_2 - mu_1) * (n[i] * j_hat);
    }

    if ((cuid == 2) && (fuid == 2)) {
      values[i] = -(mu_3 - mu_2) * (n[i] * j_hat);
    }
  }
}
                                 ...
                                 std::vector<double>& values) const
{
  ...
  for (unsigned int i = 0; i < values.size(); i++) {
    if ((cuid == 1) && (fuid == 1)) {
      values[i] = -(mu_2 - mu_1) * (n[i] * k_hat);
    }

    if ((cuid == 2) && (fuid == 2)) {
      values[i] = -(mu_3 - mu_2) * (n[i] * k_hat);
    }
  }
}
...
{
  if (((std::abs(r[0]) - d_2) < eps) && ((std::abs(r[1]) - d_2) < eps)) {
    return 1.0;
  }
