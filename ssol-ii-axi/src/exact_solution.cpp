...
double ExactSolutionSSOLIIAXI_A::value(const Point<2> & p,
	const unsigned int component) const
{
	...
	if ( r < b )
		return p[0]*(0.5*(pow(b,2)-pow(r,2))*G1 + 0.2*(pow(r,5)-pow(a,5))*G2);

	return p[0]*0.2*(pow(b,5)-pow(a,5))*G2;
}


Tensor<1, 2> ExactSolutionSSOLIIAXI_A::gradient(const Point<2> & r,
	const unsigned int component) const
{
	Point<2> grad_A;
...
void ExactSolutionSSOLIIAXI_B::vector_value_list(const std::vector<Point<2>> & r,
...
void ExactSolutionSSOLIIAXI_H::vector_value_list(const std::vector<Point<2>> & r,
	std::vector<Vector<double>> &values) const
{
	Assert(values.size() == r.size(), ExcDimensionMismatch(values.size(), r.size()));
