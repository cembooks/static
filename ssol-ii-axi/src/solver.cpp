...
void
SolverSSOLIIAXI::fill_dirichlet_stack()
{
  Solver<2>::dirichlet_stack = { { bid_axi, &dirichlet_function } };
}

void
SolverSSOLIIAXI::mark_materials()
{
  ...
  for (auto cell : Solver<2>::triangulation.active_cell_iterators()) {
    ...
    if ((cell->center().norm() < b) && (cell->center().norm() > a))
...
void
SolverSSOLIIAXI::data_slice(std::string fname)
{
