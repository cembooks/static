...
void ExactSolutionSSOLIIIAXI_B::vector_value_list(const std::vector<Point<2>> & r,
	std::vector<Vector<double>> &values) const
{
	...
	{
		B = magnetic_field_coil(0.0, r.at(i)[0], r.at(i)[1], K_0, mu_0, a2, b2)
...
void ExactSolutionSSOLIIIAXI_H::vector_value_list(const std::vector<Point<2>> & r,
	std::vector<Vector<double>> &values) const
{
	Assert(values.size() == r.size(), ExcDimensionMismatch(values.size(), r.size()));
